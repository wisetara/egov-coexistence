...
@Results({ @Result(name = Constants.VIEW, location = "contraBTC-" + Constants.VIEW + ".jsp"),
...
public class ContraBTCAction extends BaseVoucherAction {
	...
	private void setCashInHandAccount() {
		final Map<String, Object> boundaryMap = egovCommon.getCashChequeInfoForBoundary();
		addDropdownData("boundaryLevelList", (List<Boundary>) boundaryMap.get("listBndryLvl"));
		contraBean
				.setCashInHand(boundaryMap.get("cashInHand") != null ? boundaryMap.get("cashInHand").toString() : null);
	}
	...
	private void updateInstrumentVoucherReference(final List<InstrumentHeader> instrumentList) {
	...
	boolean validateInputData() {
		...
		if (showChequeNumber() && (null == contraBean.getChequeDate() || contraBean.getChequeNumber() == null
	...
	protected BigDecimal getCashBalance() {
		...
		try {
			accountBalance = egovCommon.getAccountBalance(voucherHeader.getVoucherDate(),
	...
	ContraJournalVoucher saveContraJournalVoucher(final InstrumentHeader instrumentHeader, final CVoucherHeader vh) {
	...
	Bankaccount getCashBankAccount() {
		return (Bankaccount) persistenceService.find("from Bankaccount where chartofaccounts.glcode=?",
				contraBean.getCashInHand());
	}

	@Action(value = "/contra/contraBTC-ajaxAvailableBalance")
	public String ajaxAvailableBalance() {
		if (parameters.get("accountNumberId")[0] != null && !"-1".equals(parameters.get("accountNumberId")[0])
	...
	CVoucherHeader createVoucher(final HashMap<String, Object> headerDetails,
			final List<HashMap<String, Object>> subledgerDetails, final List<HashMap<String, Object>> accountdetails) {
	...
	private List<HashMap<String, Object>> populateAccountDetails() {
		...
		accountdetails.add(populateDetailMap(bankAccount.getChartofaccounts().getGlcode(), contraBean.getAmount(),
				BigDecimal.ZERO));
	...
	public String edit() {
		...
		if (validateInputData())
			try {
				final CVoucherHeader oldVoucher = voucherService.updateVoucherHeader(voucherHeader, "Contra");
				final InstrumentVoucher instrumentVoucher = (InstrumentVoucher) persistenceService
						.find("from InstrumentVoucher where voucherHeaderId.id=?", oldVoucher.getId());
				if (instrumentVoucher == null)
					throw new ValidationException(Arrays
							.asList(new ValidationError(" System Error :Instrument is not linked with voucher", "")));
	...
	void createLedgerAndPost(final CVoucherHeader voucher) {
		try {
			...
			accountdetails.add(populateDetailMap(bankAccount.getChartofaccounts().getGlcode(), BigDecimal.ZERO,
