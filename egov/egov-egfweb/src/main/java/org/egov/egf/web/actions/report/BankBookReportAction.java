...
@ParentPackage("egov")
@Results({ @Result(name = "results", location = "bankBookReport-results.jsp"),
		...
		@Result(name = "PDF", type = "stream", location = "inputStream", params = { "inputName", "inputStream",
				"contentType", "application/pdf", "contentDisposition", "no-cache;filename=BankBookReport.pdf" }),
		@Result(name = "XLS", type = "stream", location = "inputStream", params = { "inputName", "inputStream",
				"contentType", "application/xls", "contentDisposition", "no-cache;filename=BankBookReport.xls" }) })
public class BankBookReportAction extends BaseFormAction {
	...
	private void computeTotals(final String glCode, final String fundCode,
		...
		final BankBookEntry initialOpeningBalance = getInitialAccountBalance(glCode, fundCode,
		...
		for (final BankBookEntry bankBookEntry : bankBookEntries) {
			...
			if (!rowsToBeRemoved.contains(bankBookEntry)) {// for a voucher
															// there could be
															// multiple
															// surrendered
															// cheques
					...
					if (closingBalance.longValue() < 0) {
						...
						if (isCreditOpeningBalance)
							entries.add(new BankBookEntry("Total", BigDecimal.ZERO, RECEIPT,
									closingBalance.abs().add(receiptTotal), initialBalance.abs().add(paymentTotal)));
						else
							entries.add(new BankBookEntry("Total", BigDecimal.ZERO, RECEIPT,
						...
						if (isCreditOpeningBalance)
							...
							entries.add(new BankBookEntry("Total", BigDecimal.ZERO, RECEIPT,
									initialBalance.abs().add(receiptTotal), closingBalance.abs().add(paymentTotal)));
	...
	private BankBookEntry getInitialAccountBalance(final String glCode, final String fundCode, final String deptCode) {
		...
		final BankBookEntry initialOpeningBalance = new BankBookEntry("To Opening Balance",
	...
	Map<String, Object> getParamMap() {
		...
		paramMap.put("heading", getText("bank.book.heading", new String[] { name, header.toString(),
				Constants.DDMMYYYYFORMAT2.format(startDate), Constants.DDMMYYYYFORMAT2.format(endDate) }));
	...
	public void setGetInstrumentsByVoucherIdsQuery(final String getInstrumentsByVoucherIdsQuery) {
	...
	public void setVoucherIdAndInstrumentMap(final Map<Long, List<Object[]>> voucherIdAndInstrumentMap) {
