...
import org.egov.infra.microservice.contract.DepartmentDesignation;
...
//	private DeptDesigService deptDesigService;
	...
	@Autowired
	private CreatePositionController(DeptDesigService deptDesigService, DepartmentService departmentService,
			DesignationService designationMasterService, PositionMasterService positionMasterService) {
...
//		this.positionMasterService = positionMasterService;
	...
	public List<Department> departments() {
		return this.getDepartmentsFromMs();
	}

	@ModelAttribute("designations")
	public List<Designation> designations() {
		return this.getDesignationsFromMs();
	}
	...
	public String createPosition(@Valid @ModelAttribute final Position position, final BindingResult errors,
		...
		if (position != null && position.getDeptDesig() != null && position.getDeptDesig().getDepartment() != null
			...
			DeptDesig departmentDesignation = deptDesigService.findByDepartmentAndDesignation(
					position.getDeptDesig().getDepartment().getId(), position.getDeptDesig().getDesignation().getId());
			...
			if (departmentDesignation != null) {
				...
				if (position.isPostOutsourced())
					departmentDesignation.setOutsourcedPosts((departmentDesignation.getOutsourcedPosts() != null
							? departmentDesignation.getOutsourcedPosts() + 1 : 1));
