...
function populateService(serviceCategory) {
	...
	populateserviceId({
		serviceCatId : serviceCategory.options[serviceCategory.selectedIndex].value
	});
}
	...
	success : function(o) {
		...
		if (null != result && result.length != 0) {
			...
			if (null != dom.get('fundId')) {
				dom.get('fundId').value = miscArray[0].trim();
			...
			if (null != dom.get('schemeId')) {
				...
				makeJSONCall([ "Text", "Value" ], url, {
					fundId : miscArray[0]
				}, schemeDropDownSuccessHandler, schemeDropDownFailureHandler);
			}
			if (null != dom.get('subschemeId')) {
				...
				}, subschemeDropDownSuccessHandler,
						subschemeDropDownFailureHandler);
			...
			if (null != dom.get('fundSourceId')) {
				...
				makeJSONCall([ "Text", "Value" ], url, {
					subSchemeId : miscArray[2]
				}, fundsourceDropDownSuccessHandler,
						fundsourceDropDownFailureHandler);
			...
			if (null != dom.get('functionId')) {
				dom.get('functionId').value = miscArray[5].trim();
				document.getElementById("functionId").disabled = true;
			}
			if (null != dom.get('deptId')) {
				dom.get('deptId').value = miscArray[6].trim();
...
schemeDropDownSuccessHandler = function(req, res) {
	...
	for (i = 0; i < res.results.length; i++) {
		schemeid.options[i + 1] = new Option(res.results[i].Text,
				res.results[i].Value);
	}
...
subschemeDropDownSuccessHandler = function(req, res) {
	...
	var resLength = res.results.length + 1;
	for (i = 0; i < res.results.length; i++) {
		subschemeId.options[i + 1] = new Option(res.results[i].Text,
				res.results[i].Value);
	}
	while (dropDownLength > resLength) {
...
fundsourceDropDownSuccessHandler = function(req, res) {
	...
	var resLength = res.results.length + 1;
	for (i = 0; i < res.results.length; i++) {
		fundSourceId.options[i + 1] = new Option(res.results[i].Text,
				res.results[i].Value);
	}
	while (dropDownLength > resLength) {
...
loadFinSubledgerSuccessHandler = function(req, res) {
	...
	subLedgersTable.addRow({
		SlNo : subLedgersTable.getRecordSet().getLength() + 1,
		"glcode" : "",
		"glcode.id" : "",
		"detailType.id" : "",
		"detailTypeName" : "",
	...
	for (i = 0; i < res.results.length; i++) {
		...
		updateGridSLDropdown('glcode.id', i, res.results[i].glcodeId,
				res.results[i].subledgerCode);
		var url = path
				+ '/receipts/ajaxReceiptCreate-getDetailType.action?accountCode='
				+ res.results[i].subledgerCode + '&index=' + i
				+ '&selectedDetailType=' + res.results[i].detailTypeId
				+ '&onload=flase';
