...
public class ChartOfAccounts {
	...
	@Transactional
	public boolean postTransaxtions(final Transaxtion txnList[], final String vDate)
			...
			if (Double.parseDouble(txn.getDrAmount()) == 0.0 && Double.parseDouble(txn.getCrAmount()) == 0.0) {
				...
				for (int a = 0; a < glParamList.size(); a++)
					try {
						...
						/*
						 * if(!glPrm.getDetailKey().equalsIgnoreCase("0")&&
						 * glPrm. getDetailKey().length()>0){
	...
	private boolean postInGL(final Transaxtion txnList[]) throws Exception {
		...
		for (final Transaxtion element : txnList) {
			...
			if ("0".equals(String.valueOf(txn.getDrAmount())) && "0".equals(String.valueOf(txn.getCrAmount()))) {
				if (LOGGER.isInfoEnabled())
					LOGGER.info("Comming in the zero  block");
				return false;
			} else {
		...
		for (int k = 0; k < count; k++)
			try {
				...
				if (del > 0)
					if (LOGGER.isInfoEnabled())
						LOGGER.info(
								"Records deleted from general ledger detail for GLH " + glHeaderId.get(k).toString());
			} catch (final Exception e) {
