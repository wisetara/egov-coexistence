...
public class TransactionSummaryService {
    ...
    public List<TransactionSummary> searchTransactionsForNonSubledger(Long finYear,
            Long fund, Long functn, String department, Long glcodeId) {
        TypedQuery<TransactionSummary> query = entityManager.createQuery(
                "select ts from TransactionSummary ts where ts.financialyear.id=:finYear and ts.fund.id=:fund and ts.functionid.id=:functn and ts.departmentCode=:department and ts.glcodeid.id=:glcodeId and ts.glcodeid.id not in (select glCodeId.id from CChartOfAccountDetail ) ",
    ...
    public List<TransactionSummary> searchTransactionsForSubledger(Long finYear,
            Long fund, Long functn, String department, Long glcodeId, Integer accountDetailTypeId, Integer accountDetailKeyId) {
        TypedQuery<TransactionSummary> query = entityManager.createQuery(
                "select ts from TransactionSummary ts where ts.financialyear.id=:finYear and ts.fund.id=:fund and ts.functionid.id=:functn and ts.departmentCode=:department and ts.glcodeid.id=:glcodeId and ts.accountdetailkey=:accountDetailKeyId and ts.accountdetailtype.id =:accountDetailTypeId ",
                TransactionSummary.class);
        query.setParameter("finYear", finYear);
        query.setParameter("fund", fund.intValue());
        query.setParameter("functn", functn);
    ...
    public TransactionSummary getTransactionSummary(Long glcodeId, Long accountDetailTypeId, Integer accountDetailKey) {
        TypedQuery<TransactionSummary> query = entityManager.createQuery(
                "from TransactionSummary where glcodeid.id=:glcodeid and accountdetailtype.id=:accountdetailtype and accountdetailkey=:accountdetailkey",
                TransactionSummary.class);
        query.setParameter("glcodeid", glcodeId);
