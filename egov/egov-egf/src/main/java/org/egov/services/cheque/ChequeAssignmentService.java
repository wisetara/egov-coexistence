...
public class ChequeAssignmentService extends PersistenceService<Paymentheader, Long> {
	...
	// *************IMPORTANT - CALL THIS METHOD BEFORE CALLING ANYTHING
	// ELSE**********************************************
	...
	// This method returns the Direct Bank Payments and Bill Payments for
	// Expense, Contractor and Supplier bills for mode Cheque
	...
	public List<ChequeAssignment> getExpenseBillPayments()
			throws ParseException, NumberFormatException, ApplicationException {
		if (LOGGER.isDebugEnabled())
	...
	// This method returns the consolidated mode payments that are not for
	// salary or remittance
	@SuppressWarnings("unchecked")
		...
		for (final ChequeAssignment chqAssgn : billChequeAssignmentList) {
			...
			if (detailTypeKeyAmtList != null && detailTypeKeyAmtList.size() != 0) {
				...
				detailTypeKeyAmtList = billVHIdAndGLDListForDebtitSideCCMap.get(chqAssgn.getBillVHId()) != null
						? billVHIdAndGLDListForDebtitSideCCMap.get(chqAssgn.getBillVHId()) : new ArrayList<Object[]>();
				if (detailTypeKeyAmtList == null || detailTypeKeyAmtList.size() == 0) {
					if (LOGGER.isDebugEnabled())
						LOGGER.debug(
					...
					if (LOGGER.isDebugEnabled())
						LOGGER.debug(
								"NOT YET ASSIGNED: adding to checkassignlist as detailTypeKeyAmtList is 1" + chqAssgn);
					tempExpenseChequeAssignmentList.add(chqAssgn);
				} else {
					BigDecimal deduction = BigDecimal.valueOf(0);
					// THIS dedcution will work for only one subledger .If more
					// than one you cannot have non subledger dedcution
					// as
					// you can not distribute that among multiple people
					// Also this needs same subledger entity used on debit and
					// credit side
					...
					for (final Object[] obj : detailTypeKeyAmtList) {
						...
						c.setPaidAmount(dedMap.get(key) == null ? BigDecimal.valueOf(Double.valueOf(obj[2].toString()))
								: (BigDecimal.valueOf(Double.valueOf(obj[2].toString()))).subtract(dedMap.get(key)));
	...
	// Getting only those payments for which cheques have been assigned but no
	// cheque is surrendered.
	@SuppressWarnings("unchecked")
			...
			if (detailTypeKeyAmtList != null && detailTypeKeyAmtList.size() != 0) {
				...
				if (detailTypeKeyAmtList == null || detailTypeKeyAmtList.size() == 0) {
					...
					if (payTo == null || payTo.size() == 0) {
						if (LOGGER.isDebugEnabled())
							LOGGER.debug(
									"ALREADY ASSIGNED: adding to chequeAssignlist as payto is null or 0" + chqAssgn);
						tempExpenseChequeAssignmentList.add(chqAssgn);
					}
				} else {
					Map<String, BigDecimal> dedMap = new HashMap<String, BigDecimal>();
	...
	private void getExpenseBillPaymentsWithSurrenderedCheques() throws NumberFormatException, ApplicationException {
		...
		for (final ChequeAssignment chqAssgn : billChequeAssignmentList) {
			if (LOGGER.isDebugEnabled())
				LOGGER.debug(
						"ASSIGNED BUT SURRENDARD: With surrendered cheques -  for Billvhid" + chqAssgn.getBillVHId());
			List<Object[]> detailTypeKeyAmtList = billVHIdAndgeneralLedgerDetailListMap
					.get(chqAssgn.getBillVHId().longValue()) != null
							? billVHIdAndgeneralLedgerDetailListMap.get(chqAssgn.getBillVHId().longValue())
							: new ArrayList<Object[]>();
			...
			if (detailTypeKeyAmtList != null && detailTypeKeyAmtList.size() != 0) {
				...
				if (detailTypeKeyAmtList.size() < 2) {
					...
					if (instrumentStatus == null || instrumentStatus.size() == 0
						...
						if (LOGGER.isDebugEnabled())
							LOGGER.debug("ASSIGNED BUT SURRENDARD: Adding to chequeAssignmentlist as istrumentStatus "
									+ chqAssgn);
						tempExpenseChequeAssignmentList.add(chqAssgn);
					} else {
						if (LOGGER.isDebugEnabled())
		...
		if (list != null && !list.isEmpty())
			for (final Object[] ob : list)
				map.put(ob[0].toString() + DELIMETER + ob[1].toString(),
						BigDecimal.valueOf(Double.valueOf(ob[2].toString())));
		if (LOGGER.isDebugEnabled())
